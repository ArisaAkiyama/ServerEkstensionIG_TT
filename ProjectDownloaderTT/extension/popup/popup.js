...
function createMediaItem(item, index) {
    ...
    if (thumbUrl && isVideo && (thumbUrl.includes('tiktokcdn') || thumbUrl.includes('muscdn'))) {
        thumbUrl = `${settings.serverUrl}/api/tiktok/proxy?url=${encodeURIComponent(thumbUrl)}`;
    }
...
async function downloadItem(index) {
    ...
    try {
        ...
        if (item.hasCapturedBuffer && item.filename) {
            downloadUrl = `${settings.serverUrl}/api/tiktok/download-captured/${encodeURIComponent(item.filename)}`;
        } else {
            // Fallback to proxy
            downloadUrl = `${settings.serverUrl}/api/tiktok/proxy?url=${encodeURIComponent(item.url)}`;
        }
...
async function saveItem(index) {
    ...
    try {
        ...
        if (item.hasCapturedBuffer && item.filename) {
            response = await fetch(`${settings.serverUrl}/api/tiktok/save-captured`, {
                method: 'POST',
            ...
            response = await fetch(`${settings.serverUrl}/api/tiktok/save`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    url: item.url,
